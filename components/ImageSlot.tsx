...
interface Props {
  /** Called with the Cloudinary URL once upload succeeds */
  onUrl: (url: string) => void
  /** Called immediately after crop with a local blob URL — for live preview */
...
export default function ImageSlot({
  ...
  /** Set the main preview to a new blob/file, revoking the previous one */
  const applyPreview = useCallback((blob: Blob) => {
    if (blobUrlRef.current) URL.revokeObjectURL(blobUrlRef.current)
  ...
  // ── Core upload (called after crop is confirmed) ───────────────────
  ...
  return (
    <>
      {/* Crop modal — fixed overlay, shown before upload begins */}
      <AnimatePresence>
        {showCrop && (
          <CropModal
            key="crop"
      ...
      >
        ...
        {/* Background preview (shown after crop is confirmed) */}
        {preview && (
